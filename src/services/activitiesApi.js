// services/activitiesApi.js (VERSION PROPRE ET UNIQUE)
...
export async function hrRunAI(activityId) {
  ...
  const { data } = await http.post(
    `/recommendations/${activityId}/run-ai`
  );
  return data;
}

// 🔹 3. Récupérer recommandations
...
// 🔹 5. Valider + envoyer manager
export async function hrValidateAndForward(activityId) {
  if (!activityId) throw new Error("Missing activityId");
  const { data } = await http.patch(
    `/recommendations/${activityId}/validate`
  );
  return data;
}
...
export async function managerConfirmParticipants(
  activityId,
  participantIds
) {
  if (!activityId) throw new Error("Missing activityId");
...
export async function managerNotifyEmployees(
  activityId,
  employeeIds
) {
  if (!activityId) throw new Error("Missing activityId");
  const { data } = await http.post(
    `/invitations/${activityId}/notify`,
...
export async function employeeRespond(
  invitationId,
